...
class DailyRoutineViewController: UIViewController {
    ...
    private func loadRoutineItems() {
        routineItems = dataController.getRoutineItems(for: routineType, date: selectedDate)
        dailyRoutineCollectionView.reloadData()
    }
    ...
    private func handlePrimary(for indexPath: IndexPath) {
        ...
        var progress = dataController.loadProgress(for: item, date: selectedDate)
