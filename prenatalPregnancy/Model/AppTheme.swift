...
final class AnimatedBackgroundView: UIView {
    ...
    private func layoutEmitter() {
        ...
        emitterLayer.emitterPosition = CGPoint(x: bounds.midX, y: -24)
        emitterLayer.emitterSize = CGSize(width: bounds.width * 1.35, height: 1)
    }

    private func configureSnow() {
        ...
        cell.contents = makeDot(color: UIColor.white.withAlphaComponent(0.9))?.cgImage

        emitterLayer.emitterCells = [cell]
        emitterLayer.birthRate = UIAccessibility.isReduceMotionEnabled ? 0 : 1
    }
...
final class GlobalParticleOverlayView: UIView {
    ...
    override func layoutSubviews() {
        ...
        emitterLayer.emitterSize = CGSize(width: bounds.width * 1.25, height: 1)
    }
...
extension UIWindow {
    ...
    func installGlobalParticleOverlay(theme: AppTheme) {
        let overlay: GlobalParticleOverlayView

        if let existing = subviews.first(where: { $0 is GlobalParticleOverlayView }) as? GlobalParticleOverlayView {
            overlay = existing
        } else {
            overlay = GlobalParticleOverlayView(theme: theme)
