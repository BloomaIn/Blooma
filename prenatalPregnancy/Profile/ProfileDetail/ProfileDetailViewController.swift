...
class ProfileDetailViewController: UIViewController {
    ...
    private var descriptionItem: ProfileDetailItem?
    ...
    private func setupCollectionView() {
        ...
        collectionView.register(UINib(nibName: "ProfileDetailDescriptionCollectionViewCell", bundle: nil), forCellWithReuseIdentifier: "descriptionCell")
    ...
    func createLayout() -> UICollectionViewLayout {
        ...
        return UICollectionViewCompositionalLayout { [weak self] sectionIndex, _ in
            ...
            if self.row == .permissions {
                ...
                if sectionIndex == 0 {
                    let size = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1), heightDimension: .absolute(180))
            ...
            if self.appContent != nil {
                ...
                let isIntroSection = (sectionIndex - 1) % 2 == 0
            ...
            if sectionIndex == 0 {
                let size = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1), heightDimension: .absolute(160))
                let item = NSCollectionLayoutItem(layoutSize: size)
                let group = NSCollectionLayoutGroup.vertical(layoutSize: size, subitems: [item])
                return NSCollectionLayoutSection(group: group)
            }
            
            if sectionIndex == 1 && !self.valueItems.isEmpty {
    ...
    private func generateItems() {
        ...
        if let contentId = row.contentId {
            ...
            descriptionItem = .description(title: row.title, subtitle: row.subtitle, icon: row.icon)
            valueItems = []
        ...
        case .logout:
            descriptionItem = .description(title: "Logout", subtitle: "Sign out from your account", icon: "arrow.backward.circle.fill")
            valueItems = []
...
extension ProfileDetailViewController: UICollectionViewDataSource {
    ...
    func numberOfSections(in collectionView: UICollectionView) -> Int {
        ...
        if let content = appContent {
            return (content.sections.count * 2) + 1
        }
        ...
        return valueItems.isEmpty ? 1 : 2
    }
    
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        
        if row == .permissions {
            return section == 0 ? 1 : permissionItems.count
        }
        
        guard let content = appContent else {
            return section == 0 ? 1 : valueItems.count
        }
        ...
        let contentIndex = (section - 1) / 2
        let isIntroSection = (section - 1) % 2 == 0
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "descriptionCell", for: indexPath) as! ProfileDetailDescriptionCollectionViewCell
            
            if case let .description(title, subtitle, icon) = descriptionItem {
                cell.configure(title: title, subtitle: subtitle, icon: icon, theme: theme)
            }
            
            return cell
        }
        
        if row == .permissions && indexPath.section == 1 {
        ...
        if let content = appContent {
            ...
            let contentIndex = (indexPath.section - 1) / 2
            let isIntroSection = (indexPath.section - 1) % 2 == 0
