...
class InnerCellViewController:UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    func createLayout() -> UICollectionViewCompositionalLayout {
            UICollectionViewCompositionalLayout { [weak self] sectionIndex, _ in
                guard let self = self,
                      let sectionType = SectionType(rawValue: sectionIndex) else { return nil }
                switch sectionType {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
            SectionType(rawValue: section) == nil ? 0 : 1
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
            ...
            case .grid:
                ...
                cell.theme = theme
                cell.configure(metrics: overviewMetrics, activityType: selectedActivityType , theme : theme)
                return cell
            ...
            case .healthVitals:
                ...
                cell.theme = theme
                let avgHeart = "\(vitalValue(for: "Heart Rate", fallback: "0")) \(vitalUnit(for: "Heart Rate", fallback: "bpm"))"
                let peakHeart = "\(vitalValue(for: "Peak Heart Rate", fallback: "0")) \(vitalUnit(for: "Peak Heart Rate",fallback: "bpm"))"
                let avgResp = "\(vitalValue(for: "Respiratory Rate", fallback: "0")) \(vitalUnit(for: "Respiratory Rate",fallback: "rpm"))"
                let peakResp = "\(vitalValue(for: "Peak Respiratory Rate", fallback: "0")) \(vitalUnit(for: "Peak Respiratory Rate", fallback: "rpm"))"
                cell.configureCell(avgHeartRate: avgHeart,peakHeartRate: peakHeart,respiratoryRate: avgResp,peakRespiratoryRate: peakResp,theme: theme)
                return cell
            }
        }
