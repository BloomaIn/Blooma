...
class GridStatCell: UICollectionViewCell {
    ...
    func configure(metrics: [InnerCellViewController.DetailMetricItem], activityType: String , theme : AppTheme) {
        self.theme = theme
        ...
        if type == "walking", metrics.count >= 4 {

            populateCard(metric: metrics[0],iconView: minutesImageView,titleLabel: durationLabel,valueLabel: minutesLabel,unitLabel:minutesTextLabel,cardView: durationCard)
            populateCard(metric: metrics[1],iconView: distanceImageView,titleLabel: distanceLabel,valueLabel: distanceCoveredLabel,unitLabel: distanceUnitLabel,cardView: distanceCard)
            populateCard(metric: metrics[2],iconView: stepsImageView,titleLabel: stepsTitle,valueLabel: stepsCovered,unitLabel: stepsUnitLabel,cardView: stepsCard)
            populateCard(metric: metrics[3],iconView: caloriesImageView,titleLabel: caloriesText,valueLabel:calorieCount,unitLabel: calorieUnit,cardView: calories)

        } else if (type == "exercise" || type == "yoga"), metrics.count >= 3 {
            
            populateCard(metric: metrics[0],iconView: minutesImageView,titleLabel: durationLabel,valueLabel: minutesLabel,unitLabel: minutesTextLabel,cardView: durationCard)
            populateCard(metric: metrics[1],iconView: distanceImageView,titleLabel: distanceLabel,valueLabel: distanceCoveredLabel,unitLabel: distanceUnitLabel,cardView: distanceCard)
            populateCard(metric: metrics[2],iconView: stepsImageView,titleLabel: stepsTitle,valueLabel: stepsCovered,unitLabel: stepsUnitLabel,cardView: stepsCard)
            clearCard(iconView: caloriesImageView,titleLabel: caloriesText,valueLabel: calorieCount,unitLabel: calorieUnit,cardView: calories)
        }
    }
  }
